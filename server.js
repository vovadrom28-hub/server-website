...
const db = new sqlite3.Database('./database.db', (err) => {
    if (err) console.error('Îøèáêà ÁÄ:', err.message);
});
...
http.createServer(async (req, res) => {
        ...
        req.on('end', () => {
            ...
            db.get("SELECT last_bonus, coins FROM economy WHERE nickname = ?", [data.nickname], (err, row) => {
                if (row && row.last_bonus === today) {
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    return res.end(JSON.stringify({ success: false, message: 'Áîíóñ óæå ïîëó÷åí ñåãîäíÿ!' }));
                }
                ...
                db.run("INSERT INTO economy (nickname, coins, last_bonus) VALUES (?, 50, ?) ON CONFLICT(nickname) DO UPDATE SET coins = ?, last_bonus = ?", 
                    [data.nickname, today, newCoins, today], () => {
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: true, coins: newCoins, message: 'Âàì íà÷èñëåíî 50 MineCoins!' }));
                    });
            });
        });
        return;
    ...
    if (urlPath === '/api/shop/upgrade' && req.method === 'POST') {
        ...
        req.on('end', () => {
            ...
            db.get("SELECT coins, upgrade_level FROM economy WHERE nickname = ?", [data.nickname], (err, row) => {
                ...
                if (currentLvl >= 10) {
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    return res.end(JSON.stringify({ success: false, message: 'Ìàêñèìàëüíûé óðîâåíü!' }));
                }
                let cost = currentLvl * 150;
                if (currentCoins < cost) {
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    return res.end(JSON.stringify({ success: false, message: `Íåîáõîäèìî ${cost} êîèíîâ!` }));
                }
                db.run("UPDATE economy SET coins = ?, upgrade_level = ? WHERE nickname = ?", [currentCoins - cost, currentLvl + 1, data.nickname], () => {
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: true, message: `Óðîâåíü ïîâûøåí äî ${currentLvl + 1}!` }));
                });
            });
        });
        return;
    ...
    if (urlPath === '/api/shop/exchange' && req.method === 'POST') {
        ...
        req.on('end', () => {
            ...
            db.get("SELECT coins, upgrade_level FROM economy WHERE nickname = ?", [data.nickname], (err, row) => {
                ...
                if (coins < coinCost) {
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    return res.end(JSON.stringify({ success: false, message: 'Íåäîñòàòî÷íî MineCoins!' }));
                }
                db.run("UPDATE economy SET coins = ? WHERE nickname = ?", [coins - coinCost, data.nickname], () => {
                    console.log(`[ÑÅÐÂÅÐ] /eco give ${data.nickname} ${gameCoinsReward}`);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: true, message: `Îáìåí çàâåðøåí! Îòïðàâëåíî ${gameCoinsReward} ìîíåò â èãðå.` }));
                });
            });
        });
        return;
    ...
    if (urlPath === '/api/crash/bet' && req.method === 'POST') {
        ...
        req.on('end', () => {
            let data = JSON.parse(body);
            if (crashState.status !== 'waiting') return res.end(JSON.stringify({ success: false, message: 'Èä¸ò ïîë¸ò!' }));
            db.get("SELECT coins FROM economy WHERE nickname = ?", [data.nickname], (err, row) => {
                if (!row || row.coins < data.amount || data.amount <= 0) return res.end(JSON.stringify({ success: false, message: 'Íåò êîèíîâ!' }));
...
fs.readFile(filePath, (error, content) => {
    ...
    console.log(`Ñåðâåð çàïóùåí! Ïåðåéäèòå: http://localhost:${PORT}`);
});
